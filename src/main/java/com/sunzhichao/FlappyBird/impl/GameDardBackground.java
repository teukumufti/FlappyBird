 ...
 * @author sunzhichao
 */
public class GameDardBackground {
    private static final BufferedImage darkBackgroundImg;
    ...
    static {
        darkBackgroundImg = Util.loadBufferedImage(Constant.BG_DARK_IMG_PATH);
        assert darkBackgroundImg != null;
        GROUND_HEIGHT = darkBackgroundImg.getHeight()/2;
    }
    ...
    public void draw(Graphics g,Bird bird){
        ...
        g.setColor(Constant.BG_DARK_COLOR);
        g.fillRect(0,0,Constant.FRAME_WIDTH,Constant.FRAME_HEIGHT);

        //Get the size of the background image
        int imgWidth = darkBackgroundImg.getWidth();
        int imgHeight = darkBackgroundImg.getHeight();

        int count = Constant.FRAME_WIDTH/imgWidth +2;//Number of times to draw
        for(int i=0;i<count;i++){
            g.drawImage(darkBackgroundImg,imgWidth*i-layerX,Constant.FRAME_HEIGHT-imgHeight,null);
        }
    ...
    private void movement(){
        layerX +=speed;
        if(layerX > darkBackgroundImg.getWidth()){
            layerX = 0;
        }
    }
}
